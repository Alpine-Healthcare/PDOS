...
var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  ...
  const getFromPdfs = async (hash) => {
    const response = await fetch(
      `${pdos2().gatewayURL + "/pdos"}?hash=${hash}`
    );
  ...
  const getUserMutex = async (credential_id) => {
    const mutex = await axios.get(pdos2().gatewayURL + "/pdos/mutex", {
      params: {
        credential_id
      }
    });
  ...
  const releaseMutex = async (credential_id) => {
    try {
      const releaseResp = await axios.get(pdos2().gatewayURL + "/pdos/mutex/release", { params: { credential_id } });
