...
export default class Encryption extends Module {
    ...
    async encryptNode(data) {
        ...
        return bytesToHex(aes.encrypt(utf8ToBytes(dataParsed), hexToBytes(this.accessPackage.datakey), hexToBytes(this.accessPackage.iv), "aes-256-ctr"));
    }
    async decryptNode(encryptedData) {
        if (!this.accessPackage) {
        ...
        return JSON.parse(bytesToUtf8(aes.decrypt(hexToBytes(encryptedData), hexToBytes(this.accessPackage.datakey), hexToBytes(this.accessPackage.iv), "aes-256-ctr")));
    }
    async encryptWithLit(data) {
        if (!this.core.modules.auth?.publicKey || !this.litNodeClient) {
            throw new Error("Missing publickey or litNodeClient");
        }
        const { ciphertext, dataToEncryptHash } = await encryptString({
            evmContractConditions: accessCondition(this.core.modules.auth?.publicKey),
            dataToEncrypt: data,
        }, this.litNodeClient);
        return {
            ciphertext,
    ...
    async decryptWithLit(ciphertext, dataToEncryptHash) {
