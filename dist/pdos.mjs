...
var niceErrors = {
  ...
  17: function _5(index, length) {
    return "[mobx.array] Index out of bounds, " + index + " is larger than " + length;
  },
  18: "mobx.map requires Map polyfill for the current browser. Check babel-polyfill or core-js/es6/map.js",
  19: function _6(other) {
    return "Cannot initialize from classes that inherit from Map: " + other.constructor.name;
  },
  20: function _7(other) {
    return "Cannot initialize map from " + other;
  },
  21: function _8(dataStructure) {
    return "Cannot convert to map from '" + dataStructure + "'";
  },
  22: "mobx.set requires Set polyfill for the current browser. Check babel-polyfill or core-js/es6/set.js",
  23: "It is not possible to get index atoms from arrays",
  24: function _9(thing) {
    return "Cannot obtain administration from " + thing;
  },
  25: function _10(property, name) {
    return "the entry '" + property + "' does not exist in the observable map '" + name + "'";
  },
  26: "please specify a property",
  27: function _11(property, name) {
    return "no observable property '" + property.toString() + "' found on the observable object '" + name + "'";
  },
  28: function _12(thing) {
    return "Cannot obtain atom from " + thing;
  },
  29: "Expecting some object",
  30: "invalid action stack. did you forget to finish an action?",
  31: "missing option for computed: get",
  32: function _13(name, derivation) {
    return "Cycle detected in computation " + name + ": " + derivation;
  },
  33: function _14(name) {
    return "The setter of computed value '" + name + "' is trying to update itself. Did you intend to update an _observable_ value, instead of the computed property?";
  },
  34: function _15(name) {
    return "[ComputedValue '" + name + "'] It is not possible to assign a new value to a computed value.";
  },
  35: "There are multiple, different versions of MobX active. Make sure MobX is loaded only once or use `configure({ isolateGlobalState: true })`",
  36: "isolateGlobalState should be called before MobX is running any reactions",
  37: function _16(method) {
...
function assertPropertyConfigurable(object3, prop) {
  if (process.env.NODE_ENV !== "production") {
    var descriptor = getDescriptor(object3, prop);
...
function deepEnhancer(v, _17, name) {
...
function shallowEnhancer(v, _17, name) {
...
var ObservableObjectAdministration = /* @__PURE__ */ function() {
  ...
  _proto.has_ = function has_(key) {
    var map2 = this.pendingKeys_ || (this.pendingKeys_ = /* @__PURE__ */ new Map());
...
let Notification$1 = class Notification2 extends Module {
  constructor(core, config, dependencyInjection) {
    super(core);
...
isFetchSupported && ((res) => {
  ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type) => {
    !resolvers[type] && (resolvers[type] = utils$1.isFunction(res[type]) ? (res2) => res2[type]() : (_17, config) => {
...
const hexes = /* @__PURE__ */ Array.from({ length: 256 }, (_17, i) => i.toString(16).padStart(2, "0"));
...
function validateObject(object3, validators2, optValidators = {}) {
...
function read_linear_table(w, next) {
  ...
  return m.flatMap((v, i) => {
    let [x, ...ys] = v;
    return Array(vN[i]).fill().map((_17, j) => {
      let j_dy = j * dy;
...
function init() {
  ...
  const set_add_many = (set4, v) => v.forEach((x) => set4.add(x));
  ...
  let unrestricted = r();
  const read_chunked = () => {
    let set4 = /* @__PURE__ */ new Set();
    read_sorted_array().forEach((i) => set_add_many(set4, chunks[i]));
    set_add_many(set4, read_sorted_array());
    return set4;
  };
...
function check_whole(group, unique2) {
  ...
  for (let cp of unique2) {
    ...
    if (whole) {
      let set4 = whole.M.get(cp);
      maker = maker ? maker.filter((g) => set4.has(g)) : Array_from(set4);
...
function accessListify(value) {
  if (Array.isArray(value)) {
    return value.map((set4, index) => {
      if (Array.isArray(set4)) {
        assertArgument(set4.length === 2, "invalid slot set", `value[${index}]`, set4);
        return accessSetify(set4[0], set4[1]);
      }
      assertArgument(set4 != null && typeof set4 === "object", "invalid address-slot set", "value", value);
...
function formatAccessList(value) {
  return accessListify(value).map((set4) => [set4.address, set4.storageKeys]);
}
...
const _TypedDataEncoder = class _TypedDataEncoder {
  ...
  constructor(_types2) {
    ...
    for (const [name, set4] of subtypes) {
      const st = Array.from(set4);
      st.sort();
...
function allowSingle(set4, allowed) {
  let included = [];
...
const _formatReceiptLog = object2({
  transactionIndex: getNumber,
  blockNumber: getNumber,
  transactionHash: formatHash,
  address: getAddress,
...
const _formatTransactionReceipt = object2({
...
class GasCostPlugin extends NetworkPlugin {
  ...
  constructor(effectiveBlock, costs) {
    ...
    set4("txCreate", 32e3);
    set4("txDataZero", 4);
    set4("txDataNonzero", 16);
    set4("txAccessListStorageKey", 1900);
    set4("txAccessListAddress", 2400);
...
class PollingOrphanSubscriber extends OnBlockSubscriber {
  constructor(provider, filter2) {
    ...
    __privateSet(this, _filter2, copy$2(filter2));
  }
  async _poll(blockNumber, provider) {
    throw new Error("@TODO");
  }
...
class PollingEventSubscriber {
  ...
  constructor(provider, filter2) {
    ...
    __privateSet(this, _filter3, copy$2(filter2));
...
poll_fn2 = async function(blockNumber) {
  ...
  const filter2 = copy$2(__privateGet(this, _filter3));
...
class AbstractProvider {
  ...
  async getLogs(_filter4) {
    let filter2 = this._getFilter(_filter4);
...
async function populate(signer, tx) {
  let pop2 = copyRequest(tx);
  if (pop2.to != null) {
    pop2.to = resolveAddress(pop2.to, signer);
  }
  if (pop2.from != null) {
    const from = pop2.from;
    pop2.from = Promise.all([
      signer.getAddress(),
    ...
    pop2.from = signer.getAddress();
  }
  return await resolveProperties(pop2);
}
class AbstractSigner {
  ...
  async populateCall(tx) {
    const pop2 = await populate(this, tx);
    return pop2;
  }
  async populateTransaction(tx) {
    ...
    const pop2 = await populate(this, tx);
    if (pop2.nonce == null) {
      pop2.nonce = await this.getNonce("pending");
    }
    if (pop2.gasLimit == null) {
      pop2.gasLimit = await this.estimateGas(pop2);
    }
    const network = await this.provider.getNetwork();
    if (pop2.chainId != null) {
      const chainId = getBigInt(pop2.chainId);
      assertArgument(chainId === network.chainId, "transaction chainId mismatch", "tx.chainId", tx.chainId);
    } else {
      pop2.chainId = network.chainId;
    }
    const hasEip1559 = pop2.maxFeePerGas != null || pop2.maxPriorityFeePerGas != null;
    if (pop2.gasPrice != null && (pop2.type === 2 || hasEip1559)) {
      assertArgument(false, "eip-1559 transaction do not support gasPrice", "tx", tx);
    } else if ((pop2.type === 0 || pop2.type === 1) && hasEip1559) {
      assertArgument(false, "pre-eip-1559 transaction do not support maxFeePerGas/maxPriorityFeePerGas", "tx", tx);
    }
    if ((pop2.type === 2 || pop2.type == null) && (pop2.maxFeePerGas != null && pop2.maxPriorityFeePerGas != null)) {
      pop2.type = 2;
    } else if (pop2.type === 0 || pop2.type === 1) {
      const feeData = await provider.getFeeData();
      assert(feeData.gasPrice != null, "network does not support gasPrice", "UNSUPPORTED_OPERATION", {
        operation: "getGasPrice"
      });
      if (pop2.gasPrice == null) {
        pop2.gasPrice = feeData.gasPrice;
      }
    } else {
      const feeData = await provider.getFeeData();
      if (pop2.type == null) {
        if (feeData.maxFeePerGas != null && feeData.maxPriorityFeePerGas != null) {
          pop2.type = 2;
          if (pop2.gasPrice != null) {
            const gasPrice = pop2.gasPrice;
            delete pop2.gasPrice;
            pop2.maxFeePerGas = gasPrice;
            pop2.maxPriorityFeePerGas = gasPrice;
          } else {
            if (pop2.maxFeePerGas == null) {
              pop2.maxFeePerGas = feeData.maxFeePerGas;
            }
            if (pop2.maxPriorityFeePerGas == null) {
              pop2.maxPriorityFeePerGas = feeData.maxPriorityFeePerGas;
            }
          }
      ...
      } else if (pop2.type === 2 || pop2.type === 3) {
        if (pop2.maxFeePerGas == null) {
          pop2.maxFeePerGas = feeData.maxFeePerGas;
        }
        if (pop2.maxPriorityFeePerGas == null) {
  ...
  async sendTransaction(tx) {
    ...
    const pop2 = await this.populateTransaction(tx);
    delete pop2.from;
    const txObj = Transaction.from(pop2);
...
class FilterIdEventSubscriber extends FilterIdSubscriber {
  ...
  constructor(provider, filter2) {
    ...
    __privateSet(this, _event, copy(filter2));
  }
  _recover(provider) {
...
const getFromPdfs = async (hash2) => {
  const response = await fetch(
    `${pdos().gatewayURL + "/pdos"}?hash=${hash2}`
  );
...
class Inbox extends PDFSNode {
  constructor(core, treePath, _17, hash2) {
...
const getActiveTreatments = () => {
  var _a2, _b, _c;
  const activeTreatments = ((_c = (_b = (_a2 = pdos().stores.userAccount) == null ? void 0 : _a2.edges) == null ? void 0 : _b.e_out_TreatmentManifest) == null ? void 0 : _c.treatments) ?? [];
