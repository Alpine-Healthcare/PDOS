...
describe("Encryption Module Tests", () => {
  ...
  beforeEach(() => {
    ...
    (LitJsSdk.LitNodeClient as unknown as jest.Mock).mockImplementation(
      () => mockLitNodeClient,
    );

    // Mock random bytes generation
    (getRandomBytesSync as jest.Mock).mockImplementation(
      (size: number) => new Uint8Array(size),
    );

    // Mock hex/bytes conversion
    (bytesToHex as jest.Mock).mockImplementation(
      (bytes: Uint8Array) => "mock-hex",
    );
    (hexToBytes as jest.Mock).mockImplementation(
      (hex: string) => new Uint8Array(16),
    );
    (utf8ToBytes as jest.Mock).mockImplementation(
      (str: string) => new Uint8Array(16),
    );
    (bytesToUtf8 as jest.Mock).mockImplementation(
      (bytes: Uint8Array) => '{"mock": "data"}',
    );
  ...
  describe("Portal Communication", () => {
    ...
    test("should throw error when portal emit not set", async () => {
      await expect(encryption.portal("test-type")).rejects.toThrow(
        "Portal emit not set",
      );
    });
  });
