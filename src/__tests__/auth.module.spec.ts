...
import Auth, { AuthType, InitSteps, ALPINE_HEALTHCARE } from '../modules/auth/Auth';
...
describe('Auth Module Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockContract = {
      ...
      addComputeNodeAccessForUser: jest.fn().mockResolvedValue({ wait: jest.fn() }),
  ...
  describe('Contract Interactions', () => {
    ...
    test('should update PDOS root', async () => {
      ...
      expect(mockContract.updatePDOSRoot).toHaveBeenCalledWith(mockPublicKey, newHash);
    ...
    test('should add compute node access', async () => {
      ...
      expect(mockContract.addComputeNodeAccessForUser).toHaveBeenCalledWith(computeAddress);
    ...
    test('should get users for compute node', async () => {
      ...
      expect(mockContract.getUsersForComputeNode).toHaveBeenCalledWith(computeAddress);
    });

    test('should get user compute node', async () => {
      const computeNode = await auth.getUserComputeNode();

      expect(mockContract.getUsersComputeNode).toHaveBeenCalledWith(mockPublicKey);
