...
export const getUserHashId = async (credential_id: string) => {
  const userRes = await axios.get(
    pdos().gatewayURL + "/pdos/users/" + credential_id,
  );
  const user = userRes.data;
  return user[1].hash_id;
};

export const getUserMutex = async (
...
export const releaseMutex = async (credential_id: string): Promise<boolean> => {
  try {
    const releaseResp = await axios.get(
      pdos().gatewayURL + "/pdos/mutex/release",
...
export const acquireMutexForUser = async (
  credential_id: string,
): Promise<boolean> => {
...
export const cleanupMutexForUser = async (
  credential_id: string,
): Promise<boolean> => {
